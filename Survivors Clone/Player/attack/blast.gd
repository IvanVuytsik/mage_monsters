...
func _ready():
	match level:
		...
		3: 
			...
			attack_size = 1.2 * (player.spell_size)
		...
		4: 
			...
			attack_size = 1.3 * (player.spell_size)
	...
	initial_tween.tween_property(self, "scale", Vector2(1, 1) * attack_size, 6).set_trans(Tween.TRANS_QUINT).set_ease(Tween.EASE_OUT)
	initial_tween.tween_property(self, "rotation", 60, 30).set_trans(Tween.TRANS_QUINT).set_ease(Tween.EASE_OUT)
